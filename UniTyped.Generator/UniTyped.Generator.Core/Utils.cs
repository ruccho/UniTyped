...
internal static class Utils
{
        ...
        try
        {
            if (namedType.TypeArguments.Length > 0)
            {
                ...
                for (int i = 0; i < namedType.TypeArguments.Length; i++)
                {
                    ...
                    {
                        sb.Append(context.GetTypedView(context, param, ViewUsage.SerializeField)
    ...
    public static string ExtractTypeParameters(UniTypedGeneratorContext context, ITypeSymbol type,
        bool replaceWithViewType)
    {
        if (type is not INamedTypeSymbol namedType) return "";
        var sb = GetStringBuilder();
    ...
    public static string GetFullQualifiedTypeName(UniTypedGeneratorContext context, ITypeSymbol type,
        bool replaceWithViewType, string suffix = "")
    {
    ...
    public static bool IsSerializableArrayOrList(UniTypedGeneratorContext context, ITypeSymbol symbol,
        out ITypeSymbol? elementType)
    {
        return IsArrayOrList(context, symbol, out elementType) &&
               IsSerializableAsSerializeField(context, elementType ?? throw new NullReferenceException());
    }
    ...
    public static string ToIdentifierCompatible(string value, bool applyStartCharacterConstraint = true)
    {
        Span<char> paramNameSpan = stackalloc char[value.Length];
        value.AsSpan().CopyTo(paramNameSpan);

        bool insertStartUnderscore = applyStartCharacterConstraint && !IsCSharpIdentifierStart(paramNameSpan[0]);

        Span<char> part = insertStartUnderscore ? paramNameSpan : paramNameSpan.Slice(1);

        //replace unusable characters for C# identifier name
        for (int i = 0; i < part.Length; i++)
        {
            if (!IsCSharpIdentifierPart(part[i]))
            {
                part[i] = '_';
            }
        }

        Span<char> resultSpan = stackalloc char[paramNameSpan.Length + (insertStartUnderscore ? 1 : 0)];

        if (insertStartUnderscore)
        {
            resultSpan[0] = '_';
            paramNameSpan.CopyTo(resultSpan.Slice(1));
        }
        else
        {
            paramNameSpan.CopyTo(resultSpan);
        }

        return resultSpan.ToString();
    }

    public static bool IsCSharpIdentifierStart(char c)
    {
        return c == '_' || IsCSharpIdentifierLetter(c);
    }

    public static bool IsCSharpIdentifierPart(char c)
    {
        return IsCSharpIdentifierLetter(c) ||
    ...
    public static bool IsCSharpIdentifierLetter(char c)
    {
